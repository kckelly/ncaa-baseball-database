...
def get_team_info(year: int, division: int) -> None:
    """
    ...
    teams_list = data_utils.get_schools(year, division)
    
    team_info_file_name = file_utils.get_scrape_file_name(year, division, 'team_info')
    ...
    schedule_file_name = file_utils.get_scrape_file_name(year, division, 'schedules')
    schedule_header = ['school_name', 'school_id', 'opponent_string', 'opponent_url', 'date',
    ...
    with open(team_info_file_name, 'wb') as team_info_file, \
        ...
        for team in teams_list:
            ...
            page = web_utils.get_page(team_url, 0.1, 10)

            # school website and nickname
            school_info = page.select_one('legend')
            school_nickname = school_info.text.replace(team['school_name'], '').rsplit('(', 1)[0].strip()
            try:
                    ...
                    for line in coach_info.text.strip().split('\n'):
                        if line.startswith('Name'):
                            ...
                            try:
                                ...
                                coach['coach_id'] = web_utils.get_coach_id_from_url(coach_url)
                            except AttributeError:
                                coach['coach_id'] = None
            ...
            for schedule_row in schedule_rows[starting_index::step_amount]:
                ...
                try:
                    game['opponent_url'] = game_details[1].select_one('a').attrs.get('href')
                except AttributeError:
                    game['opponent_url'] = None
